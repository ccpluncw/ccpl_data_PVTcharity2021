...
#fixedDB
  ...
  parameter <- "db"
  ParameterName <- "db"
  parameterBounds <- c(0.003, 0.003, 0.001)
  ...
  #Here I group by the three variables but add not effects for them.  This will split the data properly
    ...
    parameter <- "nSD"
    ParameterName <- "nSD"
    parameterBounds <- c(4.25, 4.25, 0.25)

    fixedNSD.425  <- rrwCreateParameterEffect(parameter = parameter, columnName = columnName, ParameterName = ParameterName, parameterBounds = parameterBounds, df.code = df.code, GroupByVariables = GroupByVariables)
#fixedValue
  ...
  #add a column for the fixed parameter.  This is needed because we will be adding another vc effect later
    ...
    parameter <- "vc"
    ParameterName <- "vConstant"
    parameterBounds <- c(-0.04, -0.04, 0.001)
  ...
  #fixedB
    ...
    parameter <- "b"
    ParameterName <- "b"
    parameterBounds <- c(51.9, 51.9, 0.01)
    ...
    #Here, we input the conditional statements for coding the dummy or effect variable.  X is the conditional, V is the value.
      ...
      parameter <- "s"
      #These are the bounds of the parameter values: c(high, low, interval)
      parameterBounds <- c(-0.067, -0.067, 0.001)

      #add them to an existing model: here we add them to the simple model to create the overall Start Effect Model
      fixedSE  <- rrwCreateParameterEffect(parameter = parameter, columnName = columnName, ParameterName = ParameterName, parameterBounds = parameterBounds, df.code = df.code, GroupByVariables = GroupByVariables)
...
### finally build fixed effects models with parameters from Exp1  ##

fixedModelList.nSDfree <- NULL
fixedModelList.nSDfree <- rrwAddParameterEffectListToRRWModel(fixedModelList.nSDfree, c(freeNSD, fixedDB, fixedB, fixedVC, fixedSE))

fixedModelList <- NULL
fixedModelList <- rrwAddParameterEffectListToRRWModel(fixedModelList, c(fixedNSD.425, fixedDB, fixedB, fixedVC, fixedSE))
...
#   fixedModelList.nSDfree=fixedModelList.nSDfree)
...
allFixedModels <- list(fixedModelList=fixedModelList)
